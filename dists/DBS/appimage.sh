...
mkdir -p "${APPDIR}/etc/skel/.local/Onu/Conf/extensions"
if [ -f "${PROJECT_ROOT}/dists/Onboarding.so" ]; then
    cp "${PROJECT_ROOT}/dists/Onboarding.so" "${APPDIR}/etc/skel/.local/Onu/Conf/extensions/"
    echo "Copied Onboarding.so to AppDir/etc/skel/.local/Onu/Conf/extensions/"
fi

cat > "${APPDIR}/AppRun" << 'EOF'
#!/bin/bash
HERE="$(dirname "$(readlink -f "$0")")"
EXTENSION_TARGET="$HOME/.local/Onu/Conf/extensions/Onboarding.so"
RUN_MARKER="$HOME/.local/share/onu/Appimg.run.done"

if [ ! -f "$EXTENSION_TARGET" ]; then
    mkdir -p "$HOME/.local/Onu/Conf/extensions"
    if [ -f "$HERE/etc/skel/.local/Onu/Conf/extensions/Onboarding.so" ]; then
        cp "$HERE/etc/skel/.local/Onu/Conf/extensions/Onboarding.so" "$EXTENSION_TARGET"
        echo "Onboarding.so has been installed to $EXTENSION_TARGET"
    fi
fi
...
exec "$HERE/usr/bin/onu" "$@"
EOF

chmod +x "${APPDIR}/AppRun"
echo "Created custom AppRun script with skel copy logic"

if [ ! -f "${APPDIR}/usr/bin/${PKG_NAME}" ]; then
